...
//
// ut-docs#2124 and ut-docs#2185: both /locations and /registers moved off
// the inline .users-form/.users-inline pattern onto the shared
// record_dialog/list_header standard (ut-docs#2010) -- their interactions
// below updated to match (tap the list-header "+" to create, tap a row to
// open it prefilled, Save/the destructive form inside the dialog), same DOM
// this suite's own categories-record-dialog-2010.spec.ts,
// locations-record-dialog-2124.spec.ts and registers-record-dialog-2185.spec.ts
// already drive.
//
// Both blocks below read the row's own data-field-active attribute (the
// record_dialog markup's one-true-signal for active/inactive state) rather
// than any inline toggle-form field -- for /registers specifically the
// location cell ALSO renders a `.muted` span ("None") whether the row is
// active or not, so a plain ".muted" presence check would have been vacuous
// (independent review finding, ut-docs#901); data-field-active sidesteps
// that ambiguity for both pages alike.
...
// occasionally has no effect the first time (ut-docs#2124/#2185's own fix
// to this file). A row click is otherwise a simple action with nothing to
// legitimately retry on real content grounds -- this exists purely to
// absorb that timing gap, bounded and visible in the loop count rather
