...
5. Sihirbaz ayrıca dükkânınızın türünü (kafe, perakende, hizmet, konaklama, pazar tezgâhı veya diğer) ve kasayı denemek için örnek veriler — küçük bir başlangıç kataloğu, 3 örnek müşteri ve 3 örnek promosyon kodu (biri %10 indirim) — yüklenip yüklenmeyeceğini sorar. İkisi de isteğe bağlıdır: hangi işletme türünü seçerseniz seçin aynı genel settir, katalog ürünleri ÖRNEK rozetiyle işaretlenir ve hepsi Ayarlar → Veriler'den istediğiniz zaman kaldırılabilir. Katalogda ayrıca yerinde %20, pakette %5 olan örnek bir vergi koduna bağlı bir "Caffè Latte" ve bir "Ham & Cheese Sandwich" bulunur; böylece yerinde/paket geçişinin KDV'yi nasıl değiştirdiğini görebilirsiniz — oran yalnızca paket oranlarını işleyen bir vergi eklentisi (örneğin Alman vergi eklentisi) kurulu ve etkin olduğunda değişir.
...
## İkinci bir kasada kullanıcılar

Bir ana kasayı takip eden bir kasada, Kullanıcılar altında yaptığınız her değişiklik — birini eklemek, PIN belirlemek, bir hesabı etkinleştirmek veya devre dışı bırakmak, rol değiştirmek — ve kendi PIN'inizi değiştirmeniz ana kasaya kaydedilir, ardından hemen burada görünür ve diğer tüm kasalara da ulaşır. Bunun için ana kasanın açık ve erişilebilir olması gerekir: değilse kasa bunu size söyler ve hiçbir şey değişmez; değişikliği ana kasada yapın veya ana kasa yeniden erişilebilir olduğunda tekrar deneyin. Rol izinleri (Kullanıcılar → İzinler) yalnızca ana kasada değiştirilir.
