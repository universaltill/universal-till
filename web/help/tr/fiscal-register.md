---
id: fiscal-register
title: Mali kayıt defteri (Almanya)
section: Dükkanı kurma
order: 112
summary: "Almanya'nın §146a Abs. 4 AO kasa bildirimi için gereken kasa ve TSE bilgilerini, işletme konumuna göre gruplandırılmış şekilde kaydedin — bildirimi Mein ELSTER üzerinden kendiniz gönderirsiniz."
routes: [/fiscal-register]
keywords: [mali, almanya, AO, TSE, ELSTER, kassensichv, bildirim, kasa, kayıt]
---

# Mali kayıt defteri (Almanya)
...
1. **Mali kayıt defteri**'ni menüden açın (yalnızca yönetici; yalnızca dükkanınızın ülkesi Almanya olarak ayarlandığında **ve** Almanya vergi eklentisi yüklü ve etkinleştirildiğinde görünür). Kayıtlar, her kasanın ait olduğu işletme konumuna göre gruplandırılır ve listenin üzerindeki arama kutusu tüm grupları aynı anda filtreler.
2. Bir kasa/TSE eşleşmesi kaydetmek için arama kutusunun yanındaki **+** düğmesine dokunun: listeden kasayı seçin, ardından sistem türünü (yaygın varsayılan değerle önceden doldurulmuş, ama düzenlenebilir), kasanın yazılım adını ve seri numarasını, TSE'nin seri numarasını, sertifika kimliğini ve türünü girin. **Edinme tarihi** zorunludur — kasanın edinildiği tarih. **Hizmete alma tarihi** (fiilen kullanılmaya başladığı tarih) isteğe bağlıdır. Bir konumun başlığı (ve adres formu) yalnızca en az bir kaydı olduğunda görünür.
...
5. Herhangi bir satıra dokunarak açın. Bir kayıt oluşturulduktan sonra düzenlenemediği için açılan şey bir form değil, kaydettiğiniz her şeyin salt okunur bir görünümüdür — bu beklenen bir durumdur, hata değil. Bir kasa hizmet dışı kaldığında, açılan bu pencerenin içinde **Hizmet dışı olarak işaretle**'ye basın. Bu, bugünün tarihini damgalar ve durumunu değiştirir — satırı asla kaldırmaz. Emekliye ayrılmış kasaları kayıtta tutmak asıl amaçtır: bu sayfanın desteklediği yükümlülük, bir kasa değiştirildiğinde sona ermez.

## Bilmekte fayda var

- Bir kasa, ömrü boyunca burada birden fazla kez görünebilir — örneğin bir donanım arızasından sonra TSE'si değiştirildiğinde. Her eşleşme kendi kaydını alır ve hiçbiri asla silinmez veya oluşturulduktan sonra düzenlenmez — ayrıntılarını yeniden görmek için istediğiniz zaman bir satıra dokunun.
