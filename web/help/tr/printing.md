...
9. **Her satıştan sonra fiş** (Ayarlar → Yazıcı), bir satış tamamlandığında kâğıt fişe ne olacağını belirler: **Her zaman yazdır** (varsayılan — her satış yazdırılır), **Müşteriye sor** (fiş ekranı **Fişi yazdır** / **Fiş istemiyorum** düğmeleriyle *Fiş ister misiniz?* diye sorar — her iki durumda da satış zaten tamamlanmıştır ve alışılmış Yazdır düğmesi altta kullanılabilir kalır) veya **Hiçbir zaman otomatik yazdırma** (kasiyer gerektiğinde fiş ekranından yazdırır). Almanya'daki mağazalar, tıpkı diğer her yerde olduğu gibi üç seçenek arasında serbestçe seçim yapabilir; ayarlar sayfası, Alman kanununun her müşteriye fiş *sunulmasını* zorunlu kıldığını (Müşteriye sor bu şartı karşılar) ve Hiçbir zaman otomatik yazdırma seçeneğinin yalnızca zaten dijital fiş sunan veya vergi dairesi muafiyeti olan bir mağazaya uygun olduğunu belirten kısa bir not gösterir — kurulu bir ülke eklentisi de seçenekleri kendi pazarının izin verdikleriyle sınırlayabilir.
