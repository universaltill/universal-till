...
1. Installieren und aktivieren Sie das Plugin **Türkei-Fiskalgerät (YN ÖKC)** unter **Plugins**, tragen Sie in seinen Einstellungen die Adresse des Geräts ein (`okc.host`, `okc.port`; die Vorgabe `127.0.0.1:4711` ist eine Brücke auf dieser Kasse) und erteilen Sie ihm die angefragte Netzwerkberechtigung — sie erlaubt dem Plugin nur diese eine Adresse. **Fiskalgerät** erscheint dann im Menü (nur Manager, sobald das Land Ihres Geschäfts auf Türkei eingestellt ist).
