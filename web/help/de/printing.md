---
id: printing
title: Belege & Drucken
section: Den Betrieb führen
order: 230
summary: Druckt Belege, Rechnungen und Tagesabschlussberichte auf einem Thermobelegdrucker oder einem gewöhnlichen Bürodrucker; Küchenbestellungen können auf einem separaten Drucker ausgegeben werden.
keywords: [drucker, beleg, küche, etiketten, papier]
---

# Belege & Drucken

Druckt Belege, Rechnungen und Tagesabschlussberichte auf einem Thermobelegdrucker oder einem gewöhnlichen Bürodrucker; Küchenbestellungen können auf einem separaten Drucker ausgegeben werden.

## Verwendung

1. Wählen Sie in den Einstellungen Ihren Drucker und dessen Typ — Thermo oder gewöhnlich.
2. Kennen Sie die Adresse Ihres Netzwerkdruckers nicht? Verwenden Sie **Drucker in diesem Netzwerk suchen**, um danach zu scannen, statt sie von Hand einzutippen — eine nur für Manager sichtbare Schaltfläche neben den Druckeradressfeldern. Sie findet nur Drucker, die sich selbst als AppSocket/JetDirect ankündigen (der übliche Typ für Netzwerk-Belegdrucker); ein reiner IPP-Drucker oder einer, der per USB angeschlossen ist, erscheint nicht und benötigt weiterhin die manuelle Eingabe von Adresse oder Gerätepfad.
3. Verwenden Sie die Testdruck-Schaltfläche, um die Verbindung zu prüfen.
4. Ein Küchendrucker kann separat eingestellt werden, damit Essensbestellungen dort gedruckt werden, wo sie zubereitet werden — dieselbe Schaltfläche **Drucker in diesem Netzwerk suchen** bietet für beide Felder einen Kandidaten an.
5. Benötigen Sie mehr als einen Küchendrucker — zum Beispiel einen Grilldrucker und einen Bardrucker? Siehe **Küchenstationen**, um Kategorien oder einzelne Artikel an ihre eigene Station zu leiten.
6. Küchenbons drucken den Bestelltyp und die Stationskopfzeile in der eingestellten Sprache der Kasse, nicht immer auf Englisch. Es gibt keine separate reine Küchensprache-Einstellung — sie folgt der einen konfigurierten Sprache der Kasse.
7. Öffnet sich die Kassenschublade nach einem Barverkauf nicht? Die meisten Schubladen sind an Pin 2 (Standard) angeschlossen — braucht Ihre Pin 5, stellen Sie dies unter **Pin der Kassenschublade** in den Druckereinstellungen ein.
8. Druckt die Kasse Währungssymbole verstümmelt — kommt `€` oder `£` als seltsame Zeichen heraus? Viele Thermodrucker verstehen kein UTF-8. Seit v0.12.12 übernimmt die Kasse dies für Sie: Ein in Euro oder Pfund eingerichtetes Geschäft mit einer westeuropäischen Sprache sendet automatisch **Westeuropa (CP858 — €/£)**, und eine bereits eingerichtete Kasse wird beim ersten Start mit dieser Version einmalig umgestellt. Ein Geschäft in Euro oder Pfund mit kroatischer, slowenischer oder slowakischer Sprache sendet auf die gleiche Weise **Mitteleuropa (Windows-1250)** (Pfund wechselt nur, wenn das Währungssymbol tatsächlich abgedeckt wird — Windows-1250 hat kein `£`, daher bleibt ein kroatisches/slowenisches/slowakisches Geschäft in Pfund auf UTF-8); Estnisch, Lettisch oder Litauisch sendet **Baltikum (Windows-1257)**; Griechisch sendet **Griechisch (Windows-1253)**; und Türkisch sendet **Türkisch (Windows-1254)** — Baltikum, Griechisch und Türkisch decken sowohl `€` als auch `£` ab. Sie benötigen **Zeichensatz** unter Einstellungen → Drucker nur, um dies zu überschreiben — zum Beispiel bei einem Drucker, der tatsächlich UTF-8 beherrscht. Dies ändert nur, was an den Belegdrucker gesendet wird — die Sprachunterstützung der Kasse auf dem Bildschirm ist davon nicht betroffen. Ein Geschäft, das in Türkischer Lira (`₺`) abrechnet, bleibt auf **UTF-8**: Keine der oben genannten Codepages kann das Lira-Zeichen drucken, daher würde ein Wechsel die verstümmelte Ausgabe nicht beheben. Arabisch- und Farsi-sprachige Kassen bleiben ebenfalls bei **UTF-8** und werden nie automatisch umgestellt — keine Single-Byte-Codepage deckt diese Alphabete ab.
9. **Beleg nach jedem Verkauf** (Einstellungen → Drucker) legt fest, was mit dem Papierbeleg geschieht, wenn ein Verkauf abgeschlossen ist: **Immer drucken** (Standard — jeder Verkauf wird gedruckt), **Kunden fragen** (der Belegbildschirm zeigt *Möchten Sie einen Beleg?* mit **Beleg drucken** / **Kein Beleg** — der Verkauf ist in beiden Fällen bereits abgeschlossen, und die gewohnte Drucken-Schaltfläche bleibt darunter verfügbar) oder **Nie automatisch drucken** (die Kassenkraft druckt bei Bedarf vom Belegbildschirm aus). Geschäfte in Deutschland sind vorerst auf Immer drucken festgelegt, solange die Belegregeln für deutsche Geschäfte noch bestätigt werden — ein installiertes Länder-Plugin kann die Auswahl ebenso auf das beschränken, was in seinem Markt zulässig ist.
