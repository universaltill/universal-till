...
3. Tippen Sie an der Kasse auf die Bezahltaste (sie zeigt die Summe), um das Zahlungsfeld zu öffnen (der Warenkorb bleibt sichtbar, während es offen ist) — die bevorzugte Methode steht vorn, und jede Schaltfläche kann eine geschätzte Gebühr anzeigen (Anbietergebühren unter Einstellungen → Zahlungen festlegen) — wählen Sie die günstigere; eine abgelehnte Karte lässt den Warenkorb unverändert. Die bevorzugte Methode ist die erste Schaltfläche des Feldes und nimmt dessen volle Breite ein — Bezahltaste, dann diese Schaltfläche belastet den genauen geschuldeten Betrag mit zwei Tipps.
