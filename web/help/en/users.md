...
9. The wizard's last screen also asks — once, and not ticked unless you tick it — whether to register this till with the Universal Till cloud right away. Tick it and, as soon as setup finishes, the till sends its device ID, your store name, and the shop type/address you entered to the Universal Till cloud marketplace, used for support, updates and licensing. Leave it unticked and nothing is sent — the till registers only when you first use the plugin store or press Register now. Either way setup never waits on the internet, and you can change your answer any time from Settings → Till registration; see [Store registration & claiming](/help/claim).
