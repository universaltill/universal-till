...
3. Every change records who made it and when, next to the status. The number shown is the short order number your customer was given (see below) — tapping it opens the full receipt in the Journal, except for an order shown here because it was taken on a *different* till (see the note below), where the number is shown but isn't a link, since this till doesn't hold that receipt itself.
