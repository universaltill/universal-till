...
9. **Receipt after each sale** (Settings → Printer) decides what happens to the paper receipt when a sale completes: **Always print** (the default — every sale prints), **Ask the customer** (the receipt screen shows *Would you like a receipt?* with **Print receipt** / **No receipt** — the sale is already complete either way, and the usual Print button stays available below), or **Never print automatically** (the cashier prints from the receipt screen when needed). Shops in Germany can choose freely among all three, same as everywhere else; the settings page shows a short note that German law requires *offering* every customer a receipt (Ask the customer satisfies this), and that Never print automatically only fits a shop that already offers a digital receipt or holds a tax-office exemption — an installed country plugin can likewise limit the choices to what its market permits.
...
Work through these in order — they cover the printer failures Settings → Printer's own **Print test receipt** button reports in detail. A few other buttons (a Journal reprint, the receipt designer's own test print, and a label print for any of the causes below) share these same underlying causes but only ever show a bare "Print failed" — if one of those fails, run **Print test receipt** in Settings → Printer to see which of the causes below it actually is. (A label print has one further failure that isn't one of these — printing on a Regular printer (system) connection — with its own specific message; see "Printing shelf/price labels" above.)
