...
This page only records data. It does not fill in, generate, or submit anything to ELSTER on your behalf, and it has no export yet (a printable/exportable summary is planned as a separate piece of work). Whether what you enter here is complete and correct for your situation is between you and your tax advisor or the Finanzamt — this page doesn't check that for you.
...
5. When a till goes out of service, open its row and press **Mark decommissioned**. This stamps today's date and changes its status — it never removes the row. Keeping retired tills on record is the point: the obligation this page supports doesn't end when a till is replaced.

## Good to know

- A till can appear more than once here over its lifetime — for example, if its TSE is swapped after a hardware failure. Each pairing gets its own entry, and none of them are ever deleted.
