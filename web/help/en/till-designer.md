...
4. **Manage categories:** the **Categories** list under the sale-screen copy shows every category — including ones you've deactivated and ones with neither quick buttons nor active items yet, which the strip itself never shows. (A category that has active items but no quick buttons does appear in the strip, with a note that no quick buttons are set up for it yet.) Tap **+ New category** to add one (name and an optional colour from the palette), the pencil on a row to rename or recolour it, and the up/down arrows to change the order the strip shows the categories in. Each row says how many quick buttons and how many catalog items the category has.
