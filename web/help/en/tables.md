...
- Assign an order to a table from the sale screen's basket, or move a held **dine-in** order onto a different table from the held-orders strip — see **Selling & checkout**. The floor plan then shows that table occupied for as long as the order stays open, and free again the moment it's paid, moved off, or the held order is deleted.
