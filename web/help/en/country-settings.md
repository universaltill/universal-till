...
- Tax is entered as a percentage — enter `19` for 19%. Half-percent rates like `8.5` (or `8,5` — a dot or a comma both work) are fine and are saved exactly — though the setup wizard prefills a new till to the nearest whole percent, so `8.5` arrives there as `9` (editable afterwards in Settings, same as any other rate).
