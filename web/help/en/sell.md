...
- **Category strip with quick buttons** — the strip of category tabs above your quick buttons: an **All** tab first, selected by default and showing every active item, then one tab per category with active items — a category that has no quick buttons yet shows a short note saying so instead of tiles. The strip always stays one row and never scrolls; once there are more categories than fit, a **…** button appears at its end (nothing changes if they all fit) — tap it to open a list of every category, each shown with its colour and how many quick buttons it has and the current one marked, and pick one to switch to it; that category's tab then takes a spot in the visible strip so you can get back to it with a single tap next time. This is the layout the press-and-hold edit mode (see **Rearranging the quick buttons** below) works on.
...
A hidden item still rings up normally — scan its barcode or find it with the sell screen's search box, either one works exactly as before. It just doesn't take up a spot in the quick-button grid or the All tab. To bring a hidden item back, see [Quick Buttons](/help/till-designer)'s "Hidden from sell screen" list — a manager or admin unhides it from there, or from the Designer's own search-and-add, which unhides an item automatically the moment you add it as a button.
