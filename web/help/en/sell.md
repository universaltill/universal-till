...
Two segments above the basket — **Dine in** and **Takeaway** — set the whole sale in one tap: every item already in the basket switches, and anything you add afterwards follows the same choice. The filled segment is the active one. This matters for tax: some items' tax rate can differ between eating in and taking away, and the shown total updates immediately when you switch. It resets to Dine in (the default) any time the basket is reset for the next customer.

**One sale, some to stay and some to go.** Under each item's name in the basket is a small pair of icons for *that item only* — 🍽️ for dine in, 🥡 for takeaway, the filled one is active. Tap 🥡 on the coffee that's leaving and 🍽️ stays lit on the cake that isn't. On a line with a quantity of 2 or more, each tap moves **one** of them to the other mode: two americanos rung up together, tap 🥡 once, and you have one to stay and one to go on two lines — tap again and the second one follows. (A weighed item, or one whose label fixed its price, moves as a whole.) Each item is taxed for its own choice, the total updates as soon as you tap, and on a tax-inclusive till the price the customer sees for an item never changes when its mode does — only the tax inside it (a tax-exclusive till adds the tax on top, so its total moves). Once the basket holds both kinds, a **Mixed** chip appears next to the whole-sale segments, which then read **All dine in** / **All takeaway**: tapping one converts every item back to a single mode.
