...
5. If you've set up tables (see **Tables & floor plan**), tap the **Table** button next to the dine-in/takeaway switch to open the table picker, then pick a table — or "No table" to clear it — and the dialog closes on its own. Picking a table reserves it straight away — the picker only offers free tables, and if a table was taken in the meantime the till says it's already occupied and keeps your current choice. The Table button only appears while the switch is on Dine in — a takeaway order can't be assigned a table, and switching an already-assigned order to Takeaway clears the assignment. Holding and later recalling a basket keeps whichever of Dine in or Takeaway was active when you held it. A held **dine-in** order keeps its table, shown as a chip on the held-orders strip; tap **Move table** on the strip to move it onto a different free table without losing it or resuming it. A held **takeaway** order has no table to move — same as a takeaway order can't be assigned one before it's held — so the strip shows no Move table control for it.
