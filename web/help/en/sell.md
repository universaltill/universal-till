...
**A per-line discount** comes off one basket line. Each line has a small discount box next to its quantity — type the amount to take off in your currency's smallest unit (e.g. type `30` to take 30p, or 30 cents, off that line) and move off the box; the basket updates immediately, the line's price drops, and the box keeps showing that same smallest-unit amount (e.g. `30`). Clear the box back to `0` to remove the discount. It can't go negative, and it only ever reduces that one line.
...
If your shop's country is Germany and an administrator has put the shop into system-of-record mode (recording real, legally-binding sales, not trial/demo), the till checks the shop's TSE (technical security device) before completing each sale **or refund** — a refund moves real money the same way a sale does, so it's covered by the identical check, not a separate one:

- **No TSE set up:** the sale or refund is refused with a message on-screen. There is no override for this state. To fix it, set up a TSE — your own hardware, your own cloud account, or a managed subscription — or ask an administrator to take the shop out of system-of-record mode until it's ready.
- **TSE set up but currently failing:** sales and refunds are paused. An owner (admin) can grant a temporary override — it needs a typed confirmation phrase, a reason, and a duration (up to 8 hours). While the override is active a banner stays on the sale and refund screens, every sale or refund taken is flagged in the audit trail, and the receipt notes that the TSE was unavailable. When the time runs out, sales and refunds pause again automatically.
...
A shop in a country whose law requires a tax-rate plugin (Germany today) gets a similar notice — "No tax-rate plugin installed" — if it isn't active yet: every sale is ringing up at one flat rate instead of the country's real dine-in/takeaway split until it's resolved. The setup wizard installs this automatically for you, so this notice is normally only seen briefly (while an install is still catching up in the background, e.g. after an offline setup) — same persistent, non-dismissable, checks-itself-every-time treatment as the fiscal-signer notice above.
