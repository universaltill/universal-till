...
3. Adjust quantity or remove lines in the basket, then tap **Pay** — the button reads the sale total once the basket has items ("Pay £x") and "Add items to pay" (disabled) while it's empty. Tapping it opens a panel over the item buttons (the basket stays visible on the left) where you pick Cash, Card, or another method, or switch to the Split tab to take more than one payment against the same sale. Close the panel (✕) to go back and adjust the basket without paying. For the fastest checkout you can skip the panel entirely: the wide ⚡ button underneath Hold Sale and Pay charges the whole sale to your shop's preferred payment method in one tap — the same exact-amount charge as that method's own button inside the panel (until a preferred method is set up, it takes cash).
