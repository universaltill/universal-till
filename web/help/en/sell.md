...
1. Scan a barcode, or find and tap the item on the sell screen: switch between category tabs — the strip above the tiles scrolls sideways when there are more categories than fit — or tap the search icon at the end of that strip to turn it into a search box and filter the current tab by name, then tap the tile to add it. The back arrow beside the search box brings the categories back and clears what you typed.
