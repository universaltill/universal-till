...
keywords: [basket, checkout, tender, pay, scan, hold, discount, camera, table, tables, modifier, customization, variant, size, customer, loyalty, promo, order type, takeaway, dine in, dine-in/takeaway prompt, before first item, at pay, suggestions, offline, sync chip, voucher, gutschein, gift card]
---
...
1. Scan a barcode, or find and tap the item on the sell screen. How the items are laid out depends on **Settings → Sell screen → How items are browsed** (see **Choosing how items are browsed** below): a grid of category tiles, every item with a row of category filters, or a strip of category tabs above your quick buttons. Whichever it is, tap the search icon at the end of the top row to turn it into a search box and search by name across every category, not just the one you were on; each match shows a category label so you know where it came from. Then tap the tile to add it. The back arrow beside the search box brings the categories (or tiles, or filters) back and clears what you typed.
...
3. Adjust a line's quantity with the **−**/**+** buttons under the quantity box, or type an exact number straight into the box itself; remove a line with its ✕ — if the line is worth anything, pick a reason (Void, Comp, or Waste) first; a cashier without permission is asked for a manager's PIN, same as any other manager override (see Reports → "Shrinkage & Loss" for the reporting side of this). On a narrow phone screen the buttons don't fit and only the box shows — type the number there instead. Then tap **Payment** — this opens a panel over the item buttons (the basket stays visible on the left) where you pick Cash, Card, or another method, or switch to the Split tab to take more than one payment against the same sale. Close the panel (✕) to go back and adjust the basket without paying. For the fastest checkout you can skip the panel entirely: the wide ⚡ button underneath Hold Sale and Payment charges the whole sale to your shop's preferred payment method in one tap — the same exact-amount charge as that method's own button inside the panel (until a preferred method is set up, it takes cash).
4. Hold a basket to serve the next customer and recall it later; refunds are under the sale history. A recalled basket that you hold again keeps its name and its place in the list — it's the same order, not a new one. Every held basket is also listed under ☰ Menu → **Open orders**, with its table, item count, total and how long it has been waiting — and the **Open orders** button beside the pay button opens that same list right here as a popup, so you can tap one to bring it back without leaving the sale screen.
5. If you've set up tables (see **Tables & floor plan**), tap the **Table** button next to the dine-in/takeaway switch to open the table picker, then pick a table — or "No table" to clear it — and the dialog closes on its own. Picking a table reserves it straight away — the picker only offers free tables, and if a table was taken in the meantime the till says it's already occupied and keeps your current choice. The Table button only appears while the sale can still seat someone — an empty basket set to Dine in, or one with at least one dine-in item. An all-takeaway order can't be assigned a table, and switching or removing the last dine-in item clears the assignment. Holding and later recalling a basket keeps whichever of Dine in or Takeaway was active when you held it. A held **dine-in** order keeps its table, shown as a chip on the held-orders strip; tap **Move table** on the strip to move it onto a different free table without losing it or resuming it. A held **all-takeaway** order has no table to move — same as it can't be assigned one before it's held — so the strip shows no Move table control for it; a held mixed order keeps and can move its table like a dine-in one.
6. To refund a completed sale, open it from Journal → sale history: pick which lines and how much of each to return (already-refunded quantity is tracked, so you can't return more than was sold), then choose cash or the original payment method to refund into. If the original sale carried a service charge, your share of it comes back automatically, in proportion to how much of the sale you're refunding — refund everything and the whole service charge comes back too. The refund total updates live on screen as you adjust quantities, so you can check the figure before you submit.
...
Two segments — **Dine in** and **Takeaway** — set the whole sale in one tap: every item already in the basket switches, and anything you add afterwards follows the same choice. The filled segment is the active one. This matters for tax: some items' tax rate can differ between eating in and taking away, and the shown total updates immediately when you switch. It resets to Dine in (the default) any time the basket is reset for the next customer. Dine-in or takeaway is a whole-sale choice — there's no separate per-item control.

**When you're asked.** A manager can change WHERE this choice sits in the flow, under Settings → **Dine-in/takeaway prompt**. By default the two segments sit at the top of the basket the whole time, same as always. A shop can instead defer the question: **before the first item**, a dialog appears the moment a new sale starts — tapping **New Sale**, or leaving the receipt after a payment — and asks Dine in or Takeaway before you can ring anything up; dismiss it without answering and it asks again the moment you try to add the first item to that basket, before it's added. Answer once (whenever it's asked) and the item lands, with every item after it following the same choice until the next new sale. Or **at Pay**: nothing is asked until you tap Payment (or the one-tap charge button), at which point the same dialog asks before the payment screen opens. Either way, once you've answered for a sale you're never asked again for it, and the top-of-basket segments are still there any time you want to change your mind mid-sale.

If a held sale was parked before a shop switched on either of the deferred prompts — or came from another till running an older version — it can still show as **Mixed** (some items dine-in, some takeaway) when you recall it: that's historical, not something you can create from the sell screen any more. The whole-sale segments still work on it exactly as before, reading **All dine in** / **All takeaway** while it's mixed — tap one to convert every item back to a single mode. A mixed sale can still have a table (there is someone sitting down); only an all-takeaway sale can't.
...
## Choosing how items are browsed

A manager picks the sell screen's layout under **Settings → Sell screen → How items are browsed**; a one-line description under the picker explains whichever option is selected, and the change shows on the sell screen's next load, no restart needed. The three choices:

- **Category tiles** (the default) — the sell screen shows one compact tile per category, with its image (if it has one — set it on the Categories page) and how many items it holds. Tap a tile to open a popup with every item in that category — every active item in your catalog, not just the ones set up as quick buttons; the ones that are quick buttons come first, in the order from the Quick Buttons designer, then the rest alphabetically. The popup has its own search box to narrow that category's items; tap an item to add it (a customizable item opens its picker as usual), and tap Close when you're done — the popup stays open so you can add several from the same category in a row.
- **All items with category filters** — every active item in your catalog in one grid (past 200 items, the first 200 with a **Load more** button beneath them), with a row of category buttons above it. Tap a category to show only its items, or **All** to see everything again; the selected category is marked with a tick, not just a colour. With many categories the row wraps onto more lines rather than cutting any off.
- **Category strip with quick buttons** — the strip of category tabs above your quick buttons: one tab per category with active items, the first one selected by default (there is no All tab in this layout — every item is still a tile under its own category, and the search box finds any of them) — a category that has no quick buttons yet shows a short note saying so instead of tiles. The strip always stays one row and never scrolls; once there are more categories than fit, a **…** button appears at its end (nothing changes if they all fit) — tap it to open a list of every category, each shown with its colour and how many quick buttons it has and the current one marked, and pick one to switch to it; that category's tab then takes a spot in the visible strip so you can get back to it with a single tap next time. This is the layout the press-and-hold edit mode (see **Rearranging the quick buttons** below) works on.

Search works the same in all three; the layout only changes how you browse when you're not searching.

## Rearranging the quick buttons

Every active item in your catalog is a quick-sale tile automatically — you don't have to add each one by hand. A new item you create, import or bring in from the cloud appears on the sale screen the moment it's active, with no extra step; the Designer's search-and-add is only for choosing which item a tile shows first and what order tiles start in.

Press and hold any item tile for about half a second (or right-click it on a mouse till) and the whole grid switches into edit mode: every tile wobbles gently in place, and each one shows three small round buttons — a pencil at the leading top corner opens that item in the catalog; a bin at the trailing top corner **deletes the item itself** from the catalog after asking you to confirm — this removes it everywhere, not just from the sell screen, so make sure that's really what you want; an eye-with-a-line-through-it at the trailing bottom corner **hides** the tile from the sell screen without touching the item — no confirmation needed, since it's easy to undo (see below). If the item has variants (Small/Regular/Large, and similar), its tile is the whole item — the bin and the eye both act on the item and every one of its variants together, never a single variant on its own. While the tiles are wobbling, drag any tile and drop it where you want it; the others slide out of the way as you go. A tile can only be moved within its own category — putting an item in a different category is done in the catalog. Tap **Done** above the grid, press Escape, or tap anywhere outside the grid to finish: the new order is saved once, at that moment, and nothing is sent while you are still dragging. On a till that mirrors another (a replica) the save is refused with a message and the grid reloads in its real order — make the change on the primary till instead. None of this needs the network; it only touches the till's own local data.

A hidden item still rings up normally — scan its barcode or find it with the sell screen's search box, either one works exactly as before. It just doesn't take up a spot among the sell screen's tiles, whichever layout you use. To bring a hidden item back, see [Quick Buttons](/help/till-designer)'s "Hidden from sell screen" list — a manager or admin unhides it from there, or from the Designer's own search-and-add, which unhides an item automatically the moment you add it as a button.

Signed in as a cashier without catalog access? All three buttons show a small lock badge on their corner before you even tap them, so you know up front a manager will need to approve the change. Tapping any of them — including Hide — still opens the same [manager-approval prompt](/help/elevation) as elsewhere in the till, rather than a silent refusal; a manager or admin can approve it right there with their PIN. Hide is the one action that skips the extra "are you sure?" confirmation step, since it's easy to undo — the manager-approval step itself still applies.
...
- If this till syncs with others (see **Multiple tills (one shop)**), the sync status chip in the left-hand menu stays put and picks up a small dot on its icon rather than disappearing or changing colour — sales keep recording locally and catch the other tills up once the connection returns.
...
If your shop has a TSE configured, a small chip near the bottom of the left-hand menu shows fiscal signing's current health at a glance — tap it to open the **Fiscal register** page for the full picture. Healthy, it shows "Fiscal signing OK"; if the most recent sale on this till completed without a TSE signature, its icon picks up a small dot and the label reads "Fiscal signing unavailable", together with how many sales today are still unsigned — a manager can open the linked page for detail, and on a cashier's own session it still shows the same status text, just not as a link. It shows nothing at all when no TSE is configured — no chip, no warning, for a shop that will never need one. This is a status indicator only, sourced entirely from the till's own record of what it has signed — it never blocks the sale screen, and it never depends on reaching the network.
