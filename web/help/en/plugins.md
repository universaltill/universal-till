...
6. If a plugin shows a red **Broken ⚠** badge on the Plugins page, its files are missing or unreadable on that till (this can happen right after a till joins the shop). You don't need to do anything: the till reinstalls it from the store automatically within about half a minute. Until then, any item whose tax rate that plugin decides can't be sold on that till — the sale screen tells you which item; remove it or wait a moment and try again.
