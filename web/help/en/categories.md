...
3. Tap a category's row (or its pencil button) to edit it. The same form opens with the name, colour, image, groups and stations filled in — change what you need and tap **Save**; every item already in the category keeps its assignment, nothing else changes.
4. Use the up and down arrow buttons on a row to reorder the list — this is also the order categories appear as tabs on the sale screen and rows on the kitchen-routing page.
...
- **Category images:** a category's image shows on its tile on the sell screen (the *Category tiles* browsing mode), on its tab in the category strip, and in the **…** list of all categories. In the form, tap one of the built-in images, upload a photo with **Take a Photo** or **Choose File** (PNG or JPEG), or tap **No image** — a built-in image and an uploaded photo replace each other. Category tiles stay compact either way: the image on top, the name and item count below. A joined till shows the built-in images too, but an uploaded photo stays on the main till for now, so there that category shows just its name.
- **Hidden from the sale screen:** untick **Show on the sale screen** in the form to keep a category and its subcategories off the sale screen's category tabs and the **…** list — useful for items you only ever scan or search for. Its items can still be sold by search, barcode scan and the **All** tab, and this list marks the category *Hidden on sale screen*. The same setting can be changed from the shop's cloud manage page, which can also give a category an icon; the till draws the icons it knows and a plain tag for any other.
