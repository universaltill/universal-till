...
- [x] Gift vouchers as a liability (multi-purpose) — selling a voucher is recorded at 0% VAT as an outstanding liability (`vouchers` / `voucher_transactions`), excluded from article revenue and the per-rate VAT bands; VAT arises when the voucher is spent, at the redeemed goods' own rates. Redeeming debits a tracked per-voucher balance (overspend refused), the day-end Z-report shows issued/redeemed counts and amounts in their own GUTSCHEINE section, and the outstanding balance is queryable per voucher (`GET /api/vouchers/{id}`). Issuing is currently API-only via `/api/pos/tender`'s `issue_vouchers` field — no cashier dialog yet ([ut-docs#1008](https://github.com/universaltill/ut-docs/issues/1008); single-purpose vouchers and DATEV posting are separate follow-ups)
