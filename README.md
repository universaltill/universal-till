...
- [x] **Table-side ordering** (customer's own phone, QR-per-table) — each enabled table's row on the floor plan shows a QR that opens `/self-order?table=<id>` on the guest's phone ([ut-docs#815](https://github.com/universaltill/ut-docs/issues/815)); the order always lands on the **Pay at counter** board. Every table gets its own basket, held in memory for the sitting and identified by a `self_order_session` cookie, so different tables order concurrently without colliding, and everyone at one table who scans its code shares that table's one order ([ut-docs#2261](https://github.com/universaltill/ut-docs/issues/2261)) — the bare walk-up kiosk keeps its single process-level basket
- [ ] **True remote/online ordering** (off-premises, over the internet) — doesn't exist; don't confuse it with the self-order kiosk or the LAN-only table-side ordering above
