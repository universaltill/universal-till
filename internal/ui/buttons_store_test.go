...
func setupFullTestDB(t *testing.T) *sql.DB {
	...
	stmts := []string{
		...
		`CREATE TABLE item_modifier_groups (id TEXT PRIMARY KEY, name TEXT, is_active INTEGER NOT NULL DEFAULT 1);`,
...
func TestButtonStoreLoad_ThumbnailFallbackPriceOrderAndModifiers(t *testing.T) {
	...
	mustExec(t, db, `INSERT INTO item_modifier_groups (id, name, is_active) VALUES ('g1','Milk',1)`)
	mustExec(t, db, `INSERT INTO item_modifier_groups (id, name, is_active) VALUES ('g2','Old',0)`)
