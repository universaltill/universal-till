...
DELETE FROM price_history
 WHERE item_id = ?
	...
	// Modifier groups are shop-wide (ADR-0101, migration 034): deleting the
	// demo item only cascades its own link/opt-out rows; a group it used
	// stays, unassigned if this was its only item. No re-anchoring step.
