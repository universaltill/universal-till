...
func TestIndexQuickPay_LabeledWithFirstActiveMethod(t *testing.T) {
	...
	if !strings.Contains(home, `data-testid="quick-pay"`) {
	...
	if !strings.Contains(home, `data-testid="quick-pay" data-method="card"`) {
		t.Fatalf("quick-pay should tender the first active method (card), body lacks it")
	}
	if !strings.Contains(home, "⚡ Card") {
		t.Fatalf("quick-pay label should be the method's live DB name (⚡ Card)")
	}
	if !strings.Contains(home, `hx-vals='{&#34;amount&#34;:0,&#34;method&#34;:&#34;card&#34;}'`) {
		t.Fatalf("quick-pay must dispatch the same amount:0/method tender POST as the pay-grid")
	}
}
...
func TestIndexQuickPay_FollowsPreferredMethodSetting(t *testing.T) {
	...
	if !strings.Contains(home, `data-testid="quick-pay" data-method="cash"`) {
		t.Fatalf("quick-pay should tender the preferred method (cash)")
	}
	if !strings.Contains(home, "⚡ Cash") {
		t.Fatalf("quick-pay label should be the preferred method's name (⚡ Cash)")
	}
}
...
func TestIndexQuickPay_ZeroStateFallsBackToCash(t *testing.T) {
	...
	if !strings.Contains(home, `data-testid="quick-pay" data-method="cash"`) {
		t.Fatalf("zero-state quick-pay should fall back to method=cash")
	}
	...
	if !strings.Contains(home, `hx-vals='{"amount":0,"method":"cash"}'`) {
		t.Fatalf("zero-state quick-pay must still dispatch the amount:0 cash tender POST")
	}
}
