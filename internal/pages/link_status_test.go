...
func TestDeriveLinkView(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{name: "linked, same version", in: linkInputs{Replica: true, HasClient: true, Client: linked, ThisVersion: "1.4.0"},
			state: linkLinked},
		// ut-docs#2738: the note compares this till with the follow target
		// (resolved by followTarget) and says whether it installs by itself.
		{name: "linked, main till newer, can follow: updating soon", in: linkInputs{Replica: true, HasClient: true,
			Client: linked, ThisVersion: "1.3.9", Target: "1.4.0", CanFollow: true}, state: linkLinked, update: linkUpdateFollowing},
		{name: "linked, main till newer, cannot follow: update needed", in: linkInputs{Replica: true, HasClient: true,
			Client: linked, ThisVersion: "1.3.9", Target: "1.4.0"}, state: linkLinked, update: linkUpdateManual},
		{name: "polling, main till newer (pinged): the note shows too", in: linkInputs{Replica: true, HasClient: true,
			Client: fleetlink.ClientStatus{Mode: fleetlink.ModePolling}, ThisVersion: "1.3.9", Target: "1.4.0", CanFollow: true},
			state: linkPolling, update: linkUpdateFollowing},
		{name: "linked, this till newer: waiting for the main till", in: linkInputs{Replica: true, HasClient: true,
			Client: linked, ThisVersion: "v1.5.0", Target: "1.4.0"}, state: linkLinked, update: linkMainUpdateWaiting},
		{name: "dev build compares nothing", in: linkInputs{Replica: true, HasClient: true, Client: linked, ThisVersion: "dev", Target: "1.4.0"},
			state: linkLinked},
		{name: "unreachable: no update note", in: linkInputs{Replica: true, WatchUnreachable: true, WatchSince: rfc(t0),
			ThisVersion: "1.3.9", Target: "1.4.0", CanFollow: true}, state: linkUnreachable, since: rfc(t0)},
...
func TestLinkChip_RendersEachState(t *testing.T) {
	...
	for _, c := range []struct {
		...
		// ut-docs#2738: short chip text, the explanation in the hint.
		{"following the main till", linkView{State: linkLinked, Update: linkUpdateFollowing, Target: "1.4.0"},
			[]string{`data-link-state="linked"`, "Updating to v1.4.0 soon", "updates itself to match the main till when no sale is open", `href="/tills"`},
			[]string{"Waiting for the main till", "needed", "Update waiting"}},
		{"polling and following", linkView{State: linkPolling, Update: linkUpdateFollowing, Target: "1.4.0"},
			[]string{`data-link-state="polling"`, "Main till: polling", "Updating to v1.4.0 soon"}, nil},
		{"manual update needed", linkView{State: linkLinked, Update: linkUpdateManual, Target: "1.4.0"},
			[]string{`data-link-state="linked"`, "Update to v1.4.0 needed", "The main till runs v1.4.0. Install it on this till in Settings",
				`href="/settings#settings-update"`},
			[]string{"soon", "Update waiting", `href="/tills"`}},
		{"waiting for the main till to update", linkView{State: linkLinked, Update: linkMainUpdateWaiting},
			[]string{"Waiting for the main till to update"}, []string{"Update waiting", "soon", "needed"}},
	} {
		t.Run(c.name, func(t *testing.T) {
