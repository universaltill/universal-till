...
func TestVariantDeactivate_PanelAndValidation(t *testing.T) {
	...
	// With panelItem: deactivates AND answers with the panel + oob item row
	// (ut-docs#1363 — never the whole table).
	rec := postForm(t, mux, "/api/catalog/variant/deactivate", "id=v1&panelItem=itm1")
	if rec.Code != http.StatusOK {
		t.Fatalf("want 200, got %d: %s", rec.Code, rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), `id="catalog-row-itm1"`) || !strings.Contains(rec.Body.String(), `hx-swap-oob="true"`) {
