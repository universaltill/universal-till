...
func TestSyncChip_ReplicaMode(t *testing.T) {
	...
	// ut-docs#1539: and it is now a rail MENU BUTTON, so the link carries the
...
func TestSyncChip_PrimaryModeWithTills(t *testing.T) {
	...
	// ut-docs#1539: "1 till", never "1 tills". The old markup concatenated the
	// count with a bare plural noun, which read "1 tills" in English and
	// "1 Kassen" in German — reported from the pilot tablet.
	if !strings.Contains(rec.Body.String(), "1 till") || strings.Contains(rec.Body.String(), "1 tills") {
...
//
// The specific regression this guards is that ut-docs#1423 moved every rail
// item to an inline SVG *because* emoji size differently per device font, and
// this one item was missed — it kept drawing itself with a ⇅ glyph inside a
// tinted pill, in a rail of plain icon buttons.
