...
func TestListItemAssets_FindsFilesInTheStableDataDir(t *testing.T) {
	...
	entries, _, err := itemAssetScope.list()
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 1 {
...
func TestSyncItemAssets_DownloadsMissingThenSkipsMatching(t *testing.T) {
	...
	itemAssetScope.pull(context.Background(), client, primary.server.URL, "bearer", nil)
...
func TestSyncItemAssets_ReDownloadsWhenSizeChanged(t *testing.T) {
	...
	itemAssetScope.pull(context.Background(), primary.server.Client(), primary.server.URL, "bearer", nil)
...
func TestSyncItemAssets_PrimaryManifestErrorIsNonFatal(t *testing.T) {
	...
	itemAssetScope.pull(context.Background(), srv.Client(), srv.URL, "bearer", nil)
