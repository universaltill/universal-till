...
func seedForPages(t *testing.T, db *sql.DB) {
	...
	stmts := []string{
		...
		`CREATE TABLE payments_archive (id TEXT NOT NULL, sale_id TEXT NOT NULL, method_id TEXT NOT NULL, amount INTEGER NOT NULL, currency TEXT NOT NULL, reference TEXT, change_given INTEGER NOT NULL, paid_at TEXT NOT NULL, tip_amount INTEGER NOT NULL, reset_batch_id TEXT NOT NULL REFERENCES reset_batches(id));`,
	...
	// 043's plugin_management addition (#706) — keep this list in sync with
	// every migration that adds a new action, so canPerform()-gated page
	// tests exercise the real seed shape.
	...
	catalog := []string{"refund", "eod_report", "cash_adjustment", "price_override", "void", "user_management", "settings", "reports", "audit", "plugin_management"}
