...
func seedForPages(t *testing.T, db *sql.DB) {
	...
	stmts := []string{
		...
		// local_date: column-identical to migration 007 (ut-docs#1342) --
		// InsertPayment always writes it now, same drift rule as above.
		...
		// local_date: column-identical to migration 007 (ut-docs#1342) --
		// InsertWorkerAllocation always writes it now, same drift rule as above.
		`CREATE TABLE worker_allocations (id TEXT NOT NULL PRIMARY KEY, source_type TEXT NOT NULL CHECK (source_type IN ('tip', 'service_charge', 'yuzde_usulu_pool')), source_id TEXT NOT NULL DEFAULT '', cashier_id TEXT NOT NULL, amount_minor INTEGER NOT NULL, allocated_at TEXT NOT NULL, note TEXT NOT NULL DEFAULT '', local_date TEXT NOT NULL DEFAULT '');`,
		...
		// local_date: column-identical to migration 007 (ut-docs#1342).
		`CREATE TABLE worker_allocations_archive (id TEXT NOT NULL, source_type TEXT NOT NULL CHECK (source_type IN ('tip', 'service_charge', 'yuzde_usulu_pool')), source_id TEXT NOT NULL DEFAULT '', cashier_id TEXT NOT NULL, amount_minor INTEGER NOT NULL, allocated_at TEXT NOT NULL, note TEXT NOT NULL DEFAULT '', reset_batch_id TEXT NOT NULL REFERENCES reset_batches(id), local_date TEXT NOT NULL DEFAULT '');`,
