...
// TestSettingsShowsMissingMandatedTaxPluginBanner covers the Settings-page
// visibility-only banner for ADR-0067 (amends ADR-0025 D4): absent for a
// non-mandated country, shown for a mandated one (DE) with no active
// tax.rate.ask plugin, and gone again once one is active. Unlike the fiscal-
// signer banner this has no system_of_record gate at all — the VAT
// dine-in/takeaway split obligation doesn't depend on which entity is the
// fiscal system of record (missingMandatedTaxPlugin's own doc comment).
// Deliberately NOT dismissable — no dismiss endpoint or control in the
// banner's markup.
func TestSettingsShowsMissingMandatedTaxPluginBanner(t *testing.T) {
	mux, _, d := newFullAuthDeps(t)
	ctx := context.Background()

	// Same minimal plugins/plugin_hooks shape as the fiscal-signer banner
	// test above — column-identical to seedForPages' (ui_smoke_test.go) for
	// data.PluginRepo.ActiveHookOwner's join to run.
	for _, stmt := range []string{
		`CREATE TABLE plugins (id TEXT PRIMARY KEY, name TEXT, version TEXT, author TEXT, is_active INTEGER DEFAULT 1, trust_level TEXT DEFAULT 'untrusted', install_state TEXT DEFAULT 'installed', runtime TEXT DEFAULT 'go', entrypoint TEXT DEFAULT '', updated_at TEXT NOT NULL DEFAULT (datetime('now')));`,
		`CREATE TABLE plugin_hooks (id TEXT PRIMARY KEY, plugin_id TEXT NOT NULL, event TEXT NOT NULL, action TEXT NOT NULL, priority INTEGER NOT NULL DEFAULT 100, is_active INTEGER NOT NULL DEFAULT 1, config_json TEXT, UNIQUE(plugin_id, event, action));`,
	} {
	...
	// Non-mandated shop (default country GB): absent regardless.
	if strings.Contains(getSettings(), `data-testid="mandated-tax-plugin-missing"`) {
		t.Fatal("banner shown for a non-mandated-country shop")
	}

	// DE (mandated), no tax.rate.ask plugin: shown, never dismissable.
	if rec := postForm(mux, "/api/settings/upsert", url.Values{"key": {"store.country"}, "value": {"DE"}}, &mgrUser); rec.Code != http.StatusNoContent {
		t.Fatalf("country upsert = %d, want 204: %s", rec.Code, rec.Body.String())
	}
	body := getSettings()
	if !strings.Contains(body, `data-testid="mandated-tax-plugin-missing"`) {
		t.Fatal("banner not shown for a DE shop with no mandated tax plugin")
	}
	if strings.Contains(body, "dismiss-tax-plugin") {
		t.Fatal("banner must not offer any dismiss endpoint/attribute")
	}

	// An active plugin holding tax.rate.ask: the banner disappears on its
	// own, no dismiss involved.
	if _, err := d.Db.ExecContext(ctx, `INSERT INTO plugins (id, name, version, install_state, entrypoint, runtime, is_active, trust_level) VALUES ('taxplugin1','Tax','1.0.0','installed','./plugin.wasm','wasm',1,'trusted')`); err != nil {
		t.Fatal(err)
	}
	if _, err := d.Db.ExecContext(ctx, `INSERT INTO plugin_hooks (id, plugin_id, event, action, is_active) VALUES ('hook-taxplugin1','taxplugin1','tax.rate.ask','tax.rate',1)`); err != nil {
		t.Fatal(err)
	}
	if strings.Contains(getSettings(), `data-testid="mandated-tax-plugin-missing"`) {
		t.Fatal("banner still shown once a mandated tax plugin is active")
	}
}
