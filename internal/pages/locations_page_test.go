...
// ut-docs#2116: /locations is one of the /admin tree's six destinations,
...
// /locations marked is-current — not the full standalone page's chrome.
func TestLocationsPage_HXRequestReturnsContentFragmentWithOOBAdminTree(t *testing.T) {
	mux, _ := newLocationsTestMux(t)
	manager := auth.User{ID: "m1", Role: "manager", DisplayName: "Manager"}
	...
	rail := body[railStart:]
	idx := strings.Index(rail, `href="/locations"`)
	if idx < 0 {
		t.Fatalf("OOB admin tree missing the /locations row: %s", rail)
	}
	...
	if !strings.Contains(rail[tagStart:tagEnd], "is-current") {
		t.Errorf("the /locations row itself is not marked is-current: %s", rail[tagStart:tagEnd])
	}
}
...
func TestLocationsPage_NonHXRequestStillRendersFullPage(t *testing.T) {
	...
	mux.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("GET /locations: %d %s", rec.Code, rec.Body.String())
	}
...
// ut-docs#2091's Vary requirement, extended to /locations now that it is a
// dual-mode destination too.
func TestLocationsPage_VaryHXRequestOnBothBranches(t *testing.T) {
	mux, _ := newLocationsTestMux(t)
	manager := auth.User{ID: "m1", Role: "manager", DisplayName: "Manager"}

	fragReq := auth.WithUser(httptest.NewRequest(http.MethodGet, "/locations", nil), manager)
	...
	fullReq := auth.WithUser(httptest.NewRequest(http.MethodGet, "/locations", nil), manager)
