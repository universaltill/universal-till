...
func registerPromotions(mux *http.ServeMux, d *common.Deps) {
	...
	renderPromotions := func(w http.ResponseWriter, r *http.Request, errKey string) {
		...
		if err != nil {
			http.Error(w, "failed to load promotions", http.StatusInternalServerError) // page-error:allow ut-docs#1458 (pending migration to httpx.RenderError — tracked follow-up card, out of #1455's scope)
			return
		}
