...
# /api/self-order/* via mux.HandleFunc(...) must never reference `d.Engine`
# (the *common.Deps receiver variable every page-registration function in
# this package uses) as CODE — comment-only lines don't count, so existing
# prose like "KioskEngine, not Engine (ut-docs#449)" never trips this. A
# genuine, deliberate exception can still reference d.Engine on a line
# carrying an inline "kiosk-engine-guard:allow <reason>" comment.
set -euo pipefail
...
# via the same mux.HandleFunc("<METHOD> /path", ...) shape used throughout
# this package. Not scoped to a self_order_*.go filename convention — the
# card explicitly asks for "any future file serving /self-order or
# /api/self-order/*", not just today's two.
...
# already in self_order_page.go, never false-positives), then look for the
# literal `d.Engine` token as code, word-bounded so it doesn't also match
# `d.KioskEngine` or some future `d.EngineXxx` field. A hit is allowed only
# if the same source line carries an explicit
# "kiosk-engine-guard:allow <reason>" comment.
violations=""
for f in ${kiosk_route_files}; do
  hits="$(grep -vE '^[[:space:]]*//' "${f}" \
    | grep -nE '(^|[^A-Za-z0-9_])d\.Engine([^A-Za-z0-9_]|$)' \
...
if [[ -n "${violations}" ]]; then
  echo "❌ kiosk-engine guard: a self-order route handler references d.Engine directly" >&2
  echo "   (kiosk handlers must use d.KioskEngine — see ut-docs#449 / ut-docs#450)" >&2
...
echo "✓ kiosk-engine guard: no self-order route handler references d.Engine"
