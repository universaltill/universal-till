...
# lives in the external ut-plugin-language-de repo. This script expects that
# repo cloned as a SIBLING directory next to this one, same relative-path
# convention scripts/audit-nav-i18n-parity.sh already uses for the same
# pack.
#
...
DE_PACK_LOCALE="${ROOT_DIR}/../ut-plugin-language-de/locales/de.json"

if [[ ! -f "${DE_PACK_LOCALE}" ]]; then
  if [[ "${UT_LOCALE_AUDIT_STRICT:-}" == "1" ]]; then
    # The dedicated CI workflow (locale-render-audit.yml) always clones the
    # pack repo first, so a missing pack here is a real precondition
    # failure, not something to skip past.
    echo "audit-locale-render: ../ut-plugin-language-de/locales/de.json not found, and UT_LOCALE_AUDIT_STRICT=1 -- the pack repo must be cloned before this script runs" >&2
    exit 1
  fi
  echo "audit-locale-render: ../ut-plugin-language-de not found next to this checkout -- skipping (dev-machine-friendly local run)."
  echo "audit-locale-render: clone https://github.com/universaltill/ut-plugin-language-de as a sibling of this repo to run this check locally, or set UT_LOCALE_AUDIT_STRICT=1 to make its absence a hard failure."
  exit 0
fi
